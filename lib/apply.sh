...
# themselves.
#
# Static layer roster (ADR 0004 D6) — the only place a layer's repo/script pairing is named.
# Deliberately just repo:script pairs, never a component id: baseline-setup may know the manifest
# *schema* and the stage *order*, never a layer's *contents* (ARCHITECTURE.md "The boundary").
# L1a -> L1b -> L1c order; L2 (meta-ai-dev) is handled separately below, it isn't --components-shaped.
LAYER_ROSTER="baseline-shell:bootstrap.sh baseline-apps:baseline-apps.sh baseline-desktop:baseline-desktop.sh"
L2_REPO="meta-ai-dev"
...
# (skip-and-report anything hidden), then invoke `<script> install --components <ids>
# [--dry-run]`. Never dies — a missing clone/manifest or a failing installer is reported and
# the caller decides whether to keep going (contract C3's "one item's failure doesn't abort the
# batch", scaled up to the engine's own layer-by-layer loop per the post-Phase-4 plan review).
apply_layer() {  # repo script selected_csv dry
  local repo="$1" script="$2" selected="$3" dry="$4"
  ...
  echo "==> $repo: install --components ${visible:-<none>}"
  local -a cmd=(bash "$dir/$script" install --components "$visible")
  [ "$dry" = 1 ] && cmd+=(--dry-run)
  if "${cmd[@]}"; then
    return 0
  fi
...
apply_plan() {  # sel_file
  local sel_file="$1"
  local entry repo script csv
  for entry in $LAYER_ROSTER; do
    repo="${entry%%:*}"; script="${entry#*:}"
...
apply_selection() {  # sel_file dry
  ...
  for entry in $LAYER_ROSTER; do
    ...
    apply_layer "$repo" "$script" "$csv" "$dry" || failed=1
  done
